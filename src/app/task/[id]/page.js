...
const PRIORITY_CONFIG = {
  high: { label: "High", bg: "bg-danger-light", text: "text-danger", border: "border-danger" },
  medium: { label: "Medium", bg: "bg-warning-light", text: "text-warning", border: "border-warning" },
  low: { label: "Low", bg: "bg-success-light", text: "text-success", border: "border-success" },
};

const DEADLINE_CONFIG = {
  red: { bg: "bg-danger-light", text: "text-danger", label: "Mendesak" },
  yellow: { bg: "bg-warning-light", text: "text-warning", label: "Segera" },
  green: { bg: "bg-success-light", text: "text-success", label: "Aman" },
};
      ...
      <div className="max-w-3xl mx-auto px-5 sm:px-8 lg:px-12 py-6">
        <div className="max-w-lg">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span
              className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${prConfig.bg} ${prConfig.text} border ${prConfig.border}`}
            >
              {prConfig.label} Priority
            </span>
            <span
              className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${dlConfig.bg} ${dlConfig.text}`}
            >
              {dlConfig.label}
            </span>
            {task.completed && (
          ...
          <div className="bg-card rounded-xl p-4 mb-4 shadow-sm">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl ${dlConfig.bg} flex items-center justify-center`}>
              ...
              <div>
                ...
                <p className={`text-xs font-medium ${dlConfig.text}`}>
                  {days < 0
          ...
          <button
            ...
            className={`w-full sm:w-auto sm:px-12 rounded-2xl py-3.5 font-semibold text-sm ${
              task.completed
